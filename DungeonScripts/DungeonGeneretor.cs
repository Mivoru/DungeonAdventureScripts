...
public class DungeonGenerator : MonoBehaviour
{
    ...
    IEnumerator FinalizeLevel()
    {
        ...
        if (surface != null)
        {
            ...
            if (triang.vertices.Length > 0)
            {
                ...
                // --- TADY BYLO CHYBÃJÕCÕ VOL¡NÕ ---
                SpawnResources();
                // ----------------------------------
            }
            else Debug.LogError("NavMesh m· 0 vrchol˘.");
        }
    }
    ...
    void SpawnResources()
    {
        Debug.Log("--- ZA»ÕN¡M SPAWNOVAT RUDY ---");

        if (resourcesToSpawn == null || resourcesToSpawn.Count == 0)
        {
            Debug.LogWarning("Seznam resourcesToSpawn je pr·zdn˝!");
            return;
        }
        ...
        foreach (var config in resourcesToSpawn)
        {
            ...
            {
                Debug.LogError($"ResourceConfig '{config.name}' nem· p¯i¯azen˝ prefab!");
                continue;
            }

            int count = UnityEngine.Random.Range(config.minCount, config.maxCount + 1);
            Debug.Log($"PokusÌm se spawnout {count} ks rudy: {config.name}");
            ...
            while (spawned < count && attempts < 500)
            {
                ...
                // ZMÃNA: PouûÌv·me spawnBlockingLayer
            ...
            Debug.Log($"-> ⁄spÏönÏ spawnuto: {spawned} / {count} ks.");
        }
    }
